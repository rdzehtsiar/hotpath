...
#[test]
fn scan_prints_single_line_enumeration_summary() {
    ...
    assert!(final_line.starts_with("files detected 2 | speed "));
...
fn scan_respects_ignore_rules_in_file_count() {
    ...
    let stdout = String::from_utf8(output.stdout).expect("stdout should be UTF-8");
    assert!(final_scan_line(&stdout).starts_with("files detected 2 | speed "));
}
...
fn final_scan_line(stdout: &str) -> &str {
    stdout
        .trim_end_matches('\n')
        .rsplit('\r')
        .next()
