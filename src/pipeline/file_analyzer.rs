...
fn has_go_generated_code_comment(window: &FileContentWindow) -> bool {
    if window.content_kind != ContentKind::Text {
        return false;
    }

    let Ok(text) = std::str::from_utf8(&window.bytes) else {
        return false;
    };
    ...
    for line in text.lines() {
        ...
        if line.starts_with("// Code generated ") && line.ends_with(" DO NOT EDIT.") {
            return true;
        }
        if line.starts_with("//") {
            continue;
...
mod tests {
    ...
    fn analyzer_classifies_generated_and_vendor_files() {
        ...
        let generated_comment_path = fixture.write(
            "client.go",
        ...
        let generated_comment = analyzer.analyze(FileAnalysisInput {
            path: generated_comment_path,
        });
